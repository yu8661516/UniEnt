...
# �7¼3 ÐÞ¸Ä2£ºÌí¼ÓR-UniEnt×¨Êô³¬²ÎÊý
parser.add_argument("--window_size", type=int, default=200, help="»¬¶¯´°¿Ú´óÐ¡")
...
def evaluate():
    ...
    if args.adaptation == "source":
        ...
        model = ostta.OSTTA(base_model, optimizer,
                            ...
                            alpha=args.alpha,
                            criterion=args.criterion)
    # �7¼3 ÐÞ¸Ä3£ºÌí¼ÓR-UniEntÊÊÅä·ÖÖ§
    elif args.adaptation == "runient":
        from runient import RUniEnt  # È·±£runient.pyÔÚÍ¬¼¶Ä¿Â¼
        # ¸´ÓÃtentµÄÄ£ÐÍÅäÖÃºÍ²ÎÊýÊÕ¼¯£¨ÈôR-UniEntÓÐ×¨ÊôÂß¼­¿ÉÌæ»»£©
        base_model = tent.configure_model(base_model)
        params, param_names = tent.collect_params(base_model)
        optimizer = setup_optimizer(params)
        # ³õÊ¼»¯R-UniEntÄ£ÐÍ
        model = RUniEnt(base_model, optimizer,
                        steps=args.steps,
                        episodic=args.episodic,
